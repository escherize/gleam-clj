...
(defn Token-schema
  ...
  []
  [:or
   [:and [:fn Name?] [:map [:value :string]]]
   [:and [:fn UpperName?] [:map [:value :string]]]
   [:and [:fn DiscardName?] [:map [:value :string]]]
   [:and [:fn Int?] [:map [:value :string]]]
   [:and [:fn Float?] [:map [:value :string]]]
   [:and [:fn String?] [:map [:value :string]]]
   [:and [:fn CommentDoc?] [:map [:value :string]]]
   [:and [:fn CommentNormal?] [:map [:value :string]]]
   [:and [:fn CommentModule?] [:map [:value :string]]]
   [:fn As?]
   [:fn Assert?]
   [:fn Auto?]
   [:fn Case?]
   ...
   [:fn RightArrow?]
   [:fn EndOfFile?]
   [:and [:fn Space?] [:map [:value :string]]]
   [:and [:fn UnterminatedString?] [:map [:value :string]]]
   [:and [:fn UnexpectedGrapheme?] [:map [:value :string]]]])

(defn to-source
  "to_source(tok: Token) -> String

   Turn a token back into its Gleam source representation."
  {:malli/schema [:=> [:cat (Token-schema)] :string]
   :gleam/src "bench/build/packages/glexer/src/glexer/token.gleam:100"}
