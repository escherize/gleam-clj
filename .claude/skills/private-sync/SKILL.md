...
- **The board tracks current work, not just future work.** Before starting
  anything non-trivial (more than one reply's worth), create the task and mark
  it In Progress (`backlog task edit <id> -s "In Progress" -a @<agent-id>`);
  mark it Done when the work is pushed. A board that only holds someday-items
  goes stale the first time someone asks "what's being worked on right now."
