...
for f in coin_change shapes sum_to jellyfish ffi_demo permissions regressions bit_patterns; do
  ./gleam-to-clj/target/debug/gleam-to-clj "gleam-src/$f.gleam" "gen/$f.clj"
done
for m in coin-change shapes sum-to jellyfish ffi-demo permissions regressions bit-patterns; do
  clojure -A:gen -M -m "$m" >/dev/null 2>&1 || { echo "FIXTURE FAILED: $m"; exit 1; }
done
echo "fixtures ok (8/8)"
