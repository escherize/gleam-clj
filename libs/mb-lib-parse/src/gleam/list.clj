...
(defn- length-loop
  ...
  {:gleam/src "stdlib-src/src/gleam/list.gleam:57"}
  [list' count']
  (if (seq list')
    (let [list' (rest list')]
      (recur list' (+' count' 1)))
...
(defn length
  ...
  {:malli/schema [:=> [:cat [:sequential :any]] :int]
   :gleam/src "stdlib-src/src/gleam/list.gleam:53"}
  [list']
  (length-loop list' 0))

(defn- count-loop
  "count_loop(list: List(a), predicate: fn(a) -> Bool, acc: Int) -> Int"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:87"}
  [list' predicate acc]
  (if (empty? list')
    acc
...
(defn count'
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:=> [:cat :any] :boolean]]
                      :int]
   :gleam/src "stdlib-src/src/gleam/list.gleam:83"}
  [list' predicate]
...
(defn- reverse-and-prepend
  "reverse_and_prepend(list prefix: List(a), to suffix: List(a)) -> List(a)
   ...
   to prepend."
  {:gleam/src "stdlib-src/src/gleam/list.gleam:131"}
  [prefix suffix]
  (if (empty? prefix)
    suffix
...
(defn reverse
  ...
  {:malli/schema [:=> [:cat [:sequential :any]] [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:122"}
  [list']
...
(defn is-empty
  ...
  {:malli/schema [:=> [:cat [:sequential :any]] :boolean]
   :gleam/src "stdlib-src/src/gleam/list.gleam:156"}
  [list']
  (= list' (list)))
...
(defn contains
  ...
  {:malli/schema [:=> [:cat [:sequential :any] :any] :boolean]
   :gleam/src "stdlib-src/src/gleam/list.gleam:187"}
  [list' elem]
  (cond
    (empty? list')
    false
...
(defn first'
  "first(list: List(a)) -> Result(a, Nil)
...
(defn rest'
  "rest(list: List(a)) -> Result(List(a), Nil)
...
(defn group
  "group(list: List(a), by key: fn(a) -> b) -> Dict(b, List(a))
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:=> [:cat :any] :any]]
   ...
   :gleam/src "stdlib-src/src/gleam/list.gleam:275"}
  [list' key]
  (dict/group key list'))

(defn- filter-loop
  "filter_loop(list: List(a), fun: fn(a) -> Bool, acc: List(a)) -> List(a)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:296"}
  [list' fun acc]
  (if (empty? list')
    (reverse acc)
...
(defn filter
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:=> [:cat :any] :boolean]]
                      [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:292"}
  [list' predicate]
  (filter-loop list' predicate (list)))

(defn- filter-map-loop
  "filter_map_loop(list: List(a), fun: fn(a) -> Result(b, c), acc: List(b)) -> List(b)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:326"}
  [list' fun acc]
  (if (empty? list')
    (reverse acc)
...
(defn filter-map
  "filter_map(list: List(a), with fun: fn(a) -> Result(b, c)) -> List(b)
...
(defn- map-loop
  "map_loop(list: List(a), fun: fn(a) -> b, acc: List(b)) -> List(b)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:355"}
  [list' fun acc]
  (if (empty? list')
    (reverse acc)
...
(defn map
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:=> [:cat :any] :any]]
                      [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:351"}
  [list' fun]
  (map-loop list' fun (list)))

(defn- map2-loop
  "map2_loop(list1: List(a), list2: List(b), fun: fn(a, b) -> c, acc: List(c)) -> List(c)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:385"}
  [list1 list2 fun acc]
...
(defn map2
  "map2(list1: List(a), list2: List(b), with fun: fn(a, b) -> c) -> List(c)
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:sequential :any] [:=> [:cat :any :any] :any]]
                      [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:377"}
  [list1 list2 fun]
  (map2-loop list1 list2 fun (list)))

(defn- map-fold-loop
  "map_fold_loop(list: List(a), fun: fn(b, a) -> #(b, c), acc: b, list_acc: List(c)) -> #(b, List(c))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:416"}
  [list' fun acc list-acc]
  (if (empty? list')
...
(defn map-fold
  "map_fold(over list: List(a), from initial: b, with fun: fn(b, a) -> #(b, c)) -> #(b, List(c))
  ...
  {:malli/schema [:=> [:cat [:sequential :any] :any [:=> [:cat :any :any] [:tuple :any :any]]]
   ...
   :gleam/src "stdlib-src/src/gleam/list.gleam:408"}
  [list' initial fun]
  (map-fold-loop list' fun initial (list)))

(defn- index-map-loop
  "index_map_loop(list: List(a), fun: fn(a, Int) -> b, index: Int, acc: List(b)) -> List(b)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:448"}
  [list' fun index acc]
  (if (empty? list')
    (reverse acc)
...
(defn index-map
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:=> [:cat :any :int] :any]]
                      [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:444"}
  [list' fun]
  (index-map-loop list' fun 0 (list)))

(defn- try-map-loop
  "try_map_loop(list: List(a), fun: fn(a) -> Result(b, c), acc: List(b)) -> Result(List(b), c)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:498"}
  [list' fun acc]
  (if (empty? list')
    (p/->Ok (reverse acc))
...
(defn try-map
  "try_map(over list: List(a), with fun: fn(a) -> Result(b, c)) -> Result(List(b), c)
...
(defn drop
  "drop(from list: List(a), up_to n: Int) -> List(a)
  ...
  {:malli/schema [:=> [:cat [:sequential :any] :int] [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:531"}
  [list' n]
  (let [subject (<= n 0)]
    (if subject
      list'
...
(defn- take-loop
  "take_loop(list: List(a), n: Int, acc: List(a)) -> List(a)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:564"}
  [list' n acc]
  (let [subject (<= n 0)]
    (if subject
      (reverse acc)
...
(defn take
  "take(from list: List(a), up_to n: Int) -> List(a)
   ...
   ```"
  {:malli/schema [:=> [:cat [:sequential :any] :int] [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:560"}
  [list' n]
...
(defn new*
  "new() -> List(a)
   ...
   ```"
  {:malli/schema [:=> [:cat] [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:583"}
  []
  (list))
...
(defn wrap
  ...
  {:malli/schema [:=> [:cat :any] [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:604"}
  [item]
  (list item))

(defn- append-loop
  "append_loop(first: List(a), second: List(a)) -> List(a)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:624"}
  [first' second]
  (if (empty? first')
    second
...
(defn append
  "append(first: List(a), second: List(a)) -> List(a)
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:sequential :any]]
                      [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:620"}
  [first' second]
  (append-loop (reverse first') second))

(defn prepend
  "prepend(to list: List(a), this item: a) -> List(a)
  ...
  {:malli/schema [:=> [:cat [:sequential :any] :any] [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:644"}
  [list' item]
  (list* item list'))

(defn- flatten-loop
  "flatten_loop(lists: List(List(a)), acc: List(a)) -> List(a)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:664"}
  [lists acc]
  (if (empty? lists)
    (reverse acc)
...
(defn flatten
  ...
  {:malli/schema [:=> [:cat [:sequential [:sequential :any]]]
                      [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:660"}
  [lists]
...
(defn flat-map
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:=> [:cat :any] [:sequential :any]]]
                      [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:680"}
  [list' fun]
  (flatten (map list' fun)))

(defn fold
  "fold(over list: List(a), from initial: b, with fun: fn(b, a) -> b) -> b
  ...
  {:malli/schema [:=> [:cat [:sequential :any] :any [:=> [:cat :any :any] :any]]
                      :any]
   :gleam/src "stdlib-src/src/gleam/list.gleam:692"}
  [list' initial fun]
  (if (empty? list')
    initial
    (let [first' (first list') rest' (rest list')]
...
(defn fold-right
  "fold_right(over list: List(a), from initial: b, with fun: fn(b, a) -> b) -> b
  ...
  {:malli/schema [:=> [:cat [:sequential :any] :any [:=> [:cat :any :any] :any]]
                      :any]
   :gleam/src "stdlib-src/src/gleam/list.gleam:714"}
  [list' initial fun]
  (if (empty? list')
    initial
    (let [first' (first list') rest' (rest list')]
...
(defn- index-fold-loop
  "index_fold_loop(over: List(a), acc: b, with: fn(b, a, Int) -> b, index: Int) -> b"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:751"}
  [over acc with index]
  (if (empty? over)
    acc
...
(defn index-fold
  "index_fold(over list: List(a), from initial: b, with fun: fn(b, a, Int) -> b) -> b
  ...
  {:malli/schema [:=> [:cat [:sequential :any] :any [:=> [:cat :any :any :int] :any]]
                      :any]
   :gleam/src "stdlib-src/src/gleam/list.gleam:743"}
  [list' initial fun]
  (index-fold-loop list' initial fun 0))

(defn try-fold
  "try_fold(over list: List(a), from initial: b, with fun: fn(b, a) -> Result(b, c)) -> Result(b, c)
...
(defn fold-until
  "fold_until(over list: List(a), from initial: b, with fun: fn(b, a) -> ContinueOrStop(b)) -> b
...
(defn all
  "all(in list: List(a), satisfying predicate: fn(a) -> Bool) -> Bool
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:=> [:cat :any] :boolean]]
                      :boolean]
   :gleam/src "stdlib-src/src/gleam/list.gleam:921"}
  [list' predicate]
  (if (empty? list')
    true
    (let [first' (first list') rest' (rest list') subject (predicate first')]
...
(defn any
  "any(in list: List(a), satisfying predicate: fn(a) -> Bool) -> Bool
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:=> [:cat :any] :boolean]]
                      :boolean]
   :gleam/src "stdlib-src/src/gleam/list.gleam:954"}
  [list' predicate]
  (if (empty? list')
    false
    (let [first' (first list') rest' (rest list') subject (predicate first')]
...
(defn- zip-loop
  "zip_loop(one: List(a), other: List(b), acc: List(#(a, b))) -> List(#(a, b))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:992"}
  [one other acc]
...
(defn zip
  "zip(list: List(a), with other: List(b)) -> List(#(a, b))
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:sequential :any]]
   ...
   :gleam/src "stdlib-src/src/gleam/list.gleam:988"}
  [list' other]
  (zip-loop list' other (list)))

(defn- strict-zip-loop
  "strict_zip_loop(one: List(a), other: List(b), acc: List(#(a, b))) -> Result(List(#(a, b)), Nil)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1029"}
  [one other acc]
  (cond
...
(defn strict-zip
  "strict_zip(list: List(a), with other: List(b)) -> Result(List(#(a, b)), Nil)
...
(defn- unzip-loop
  "unzip_loop(input: List(#(a, b)), one: List(a), other: List(b)) -> #(List(a), List(b))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1058"}
  [input one other]
  (if (empty? input)
...
(defn unzip
  ...
  {:malli/schema [:=> [:cat [:sequential [:tuple :any :any]]]
   ...
   :gleam/src "stdlib-src/src/gleam/list.gleam:1054"}
  [input]
  (unzip-loop input (list) (list)))

(defn- intersperse-loop
  "intersperse_loop(list: List(a), separator: a, acc: List(a)) -> List(a)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1091"}
  [list' separator acc]
  (if (empty? list')
    (reverse acc)
    (let [first' (first list') rest' (rest list')]
...
(defn intersperse
  "intersperse(list: List(a), with elem: a) -> List(a)
   ...
   ```"
  {:malli/schema [:=> [:cat [:sequential :any] :any] [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1084"}
  [list' elem]
...
(defn- unique-loop
  "unique_loop(list: List(a), seen: Dict(a, Nil), acc: List(a)) -> List(a)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1113"}
  [list' seen acc]
  (if (empty? list')
    (reverse acc)
...
(defn unique
  "unique(list: List(a)) -> List(a)
   ...
   ```"
  {:malli/schema [:=> [:cat [:sequential :any]] [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1109"}
  [list']
  (unique-loop list' (dict/new*) (list)))

(defn- merge-descendings
  "merge_descendings(list1: List(a), list2: List(a), compare: fn(a, a) -> Order, acc: List(a)) -> List(a)
  ...
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1374"}
...
(defn- merge-descending-pairs
  "merge_descending_pairs(sequences: List(List(a)), compare: fn(a, a) -> Order, acc: List(List(a))) -> List(List(a))

   This is the same as merge_ascending_pairs but flipped for descending lists."
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1322"}
...
(defn- merge-ascendings
  "merge_ascendings(list1: List(a), list2: List(a), compare: fn(a, a) -> Order, acc: List(a)) -> List(a)
   ...
   returning it but that would end up being less efficient; so the merging
   algorithm has to play around this."
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1347"}
  [list1 list2 compare acc]
  (cond
    (empty? list1)
    (let [list' list2]
...
(defn- merge-ascending-pairs
  "merge_ascending_pairs(sequences: List(List(a)), compare: fn(a, a) -> Order, acc: List(List(a))) -> List(List(a))
  ...
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1300"}
  [sequences compare acc]
  (cond
    (empty? sequences)
    (reverse acc)
...
(defn- merge-all
  "merge_all(sequences: List(List(a)), direction: Sorting, compare: fn(a, a) -> Order) -> List(a)
   ...
   ascending order."
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1266"}
...
(defn- sequences
  "sequences(list: List(a), compare: fn(a, a) -> Order, growing: List(a), direction: Sorting, prev: a, acc: List(List(a))) -> List(List(a))
  ...
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1199"}
...
(defn sort
  "sort(list: List(a), by compare: fn(a, a) -> Order) -> List(a)
...
(defn- repeat-loop
  "repeat_loop(item: a, times: Int, acc: List(a)) -> List(a)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1407"}
  [item times acc]
  (let [subject (<= times 0)]
    (if subject acc (recur item (-' times 1) (list* item acc)))))

(defn repeat
  "repeat(item a: a, times times: Int) -> List(a)
  ...
  {:malli/schema [:=> [:cat :any :int] [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1403"}
  [a times]
  (repeat-loop a times (list)))

(defn- split-loop
  "split_loop(list: List(a), n: Int, taken: List(a)) -> #(List(a), List(a))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1437"}
  [list' n taken]
  (let [subject (<= n 0)]
    (if subject
      [(reverse taken) list']
...
(defn split
  "split(list list: List(a), at index: Int) -> #(List(a), List(a))
  ...
  {:malli/schema [:=> [:cat [:sequential :any] :int]
                      [:tuple [:sequential :any] [:sequential :any]]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1433"}
  [list' index]
  (split-loop list' index (list)))

(defn- split-while-loop
  "split_while_loop(list: List(a), f: fn(a) -> Bool, acc: List(a)) -> #(List(a), List(a))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1473"}
  [list' f acc]
  (if (empty? list')
    [(reverse acc) (list)]
...
(defn split-while
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:=> [:cat :any] :boolean]]
                      [:tuple [:sequential :any] [:sequential :any]]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1466"}
  [list' predicate]
  (split-while-loop list' predicate (list)))

(defn key-find
  "key_find(in keyword_list: List(#(a, b)), find desired_key: a) -> Result(b, Nil)
...
(defn key-filter
  "key_filter(in keyword_list: List(#(a, b)), find desired_key: a) -> List(b)
  ...
  {:malli/schema [:=> [:cat [:sequential [:tuple :any :any]] :any]
                      [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1539"}
  [keyword-list desired-key]
...
(defn- key-pop-loop
  "key_pop_loop(list: List(#(a, b)), key: a, checked: List(#(a, b))) -> Result(#(b, List(#(a, b))), Nil)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1579"}
  [list' key checked]
  (cond
    (empty? list')
    (p/->Error nil)
...
(defn key-pop
  "key_pop(list: List(#(a, b)), key: a) -> Result(#(b, List(#(a, b))), Nil)
...
(defn- key-set-loop
  "key_set_loop(list: List(#(a, b)), key: a, value: b, inspected: List(#(a, b))) -> List(#(a, b))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1612"}
  [list' key value inspected]
  (cond
    (and (seq list') (= (nth (first list') 0) key))
...
(defn key-set
  "key_set(list: List(#(a, b)), key: a, value: b) -> List(#(a, b))
  ...
  {:malli/schema [:=> [:cat [:sequential [:tuple :any :any]] :any :any]
                      [:sequential [:tuple :any :any]]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1608"}
  [list' key value]
  (key-set-loop list' key value (list)))

(defn each
  "each(list: List(a), f: fn(a) -> b) -> Nil
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:=> [:cat :any] :any]] :nil]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1639"}
  [list' f]
  (if (empty? list')
    nil
    (let [first' (first list') rest' (rest list')]
...
(defn try-each
  "try_each(over list: List(a), with fun: fn(a) -> Result(b, c)) -> Result(Nil, c)
...
(defn- partition-loop
  "partition_loop(list: List(a), categorise: fn(a) -> Bool, trues: List(a), falses: List(a)) -> #(List(a), List(a))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1694"}
  [list' categorise trues falses]
  (if (empty? list')
...
(defn partition
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:=> [:cat :any] :boolean]]
                      [:tuple [:sequential :any] [:sequential :any]]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1687"}
  [list' categorise]
  (partition-loop list' categorise (list) (list)))

(declare permutation-prepend permutation-zip permutations)

(defn- permutation-prepend
  "permutation_prepend(el: a, permutations: List(List(a)), list_1: List(a), list_2: List(a), acc: List(List(a))) -> List(List(a))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1738"}
  [el permutations list-1 list-2 acc]
...
(defn- permutation-zip
  "permutation_zip(list: List(a), rest: List(a), acc: List(List(a))) -> List(List(a))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1720"}
  [list' rest' acc]
  (if (empty? list')
    (reverse acc)
...
(defn permutations
  ...
  {:malli/schema [:=> [:cat [:sequential :any]]
   ...
   :gleam/src "stdlib-src/src/gleam/list.gleam:1713"}
  [list']
  (if (empty? list')
    (list (list))
    (let [l list']
...
(defn- window-loop
  "window_loop(acc: List(List(a)), list: List(a), n: Int) -> List(List(a))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1771"}
  [acc list' n]
  (let [window (take list' n) subject (= (length window) n)]
    (if subject (recur (list* window acc) (drop list' 1) n) (reverse acc))))

(defn window
  "window(list: List(a), by n: Int) -> List(List(a))
  ...
  {:malli/schema [:=> [:cat [:sequential :any] :int]
                      [:sequential [:sequential :any]]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1764"}
  [list' n]
  (let [subject (<= n 0)]
    (if subject (list) (window-loop (list) list' n))))

(defn window-by-2
  "window_by_2(list: List(a)) -> List(#(a, a))
  ...
  {:malli/schema [:=> [:cat [:sequential :any]]
                      [:sequential [:tuple :any :any]]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1792"}
  [list']
...
(defn drop-while
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:=> [:cat :any] :boolean]]
                      [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1804"}
  [list' predicate]
  (if (empty? list')
    (list)
    (let [first' (first list') rest' (rest list') subject (predicate first')]
...
(defn- take-while-loop
  "take_while_loop(list: List(a), predicate: fn(a) -> Bool, acc: List(a)) -> List(a)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1833"}
  [list' predicate acc]
  (if (empty? list')
    (reverse acc)
    (let [first' (first list') rest' (rest list') subject (predicate first')]
...
(defn take-while
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:=> [:cat :any] :boolean]]
                      [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1826"}
  [list' predicate]
  (take-while-loop list' predicate (list)))

(defn- chunk-loop
  "chunk_loop(list: List(a), f: fn(a) -> b, previous_key: b, current_chunk: List(a), acc: List(List(a))) -> List(List(a))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1865"}
  [list' f previous-key current-chunk acc]
  (if (seq list')
...
(defn chunk
  "chunk(in list: List(a), by f: fn(a) -> b) -> List(List(a))
  ...
  {:malli/schema [:=> [:cat [:sequential :any] [:=> [:cat :any] :any]]
                      [:sequential [:sequential :any]]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1858"}
  [list' f]
  (if (empty? list')
    (list)
    (let [first' (first list') rest' (rest list')]
...
(defn- sized-chunk-loop
  "sized_chunk_loop(list: List(a), count: Int, left: Int, current_chunk: List(a), acc: List(List(a))) -> List(List(a))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1910"}
  [list' count' left current-chunk acc]
  (if (empty? list')
...
(defn sized-chunk
  "sized_chunk(in list: List(a), into count: Int) -> List(List(a))
  ...
  {:malli/schema [:=> [:cat [:sequential :any] :int]
                      [:sequential [:sequential :any]]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1906"}
  [list' count']
  (sized-chunk-loop list' count' count' (list) (list)))

(defn reduce
  "reduce(over list: List(a), with fun: fn(a, a) -> a) -> Result(a, Nil)
...
(defn- scan-loop
  "scan_loop(list: List(a), accumulator: b, accumulated: List(b), fun: fn(b, a) -> b) -> List(b)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:1976"}
  [list' accumulator accumulated fun]
  (if (empty? list')
    (reverse accumulated)
...
(defn scan
  "scan(over list: List(a), from initial: b, with fun: fn(b, a) -> b) -> List(b)
  ...
  {:malli/schema [:=> [:cat [:sequential :any] :any [:=> [:cat :any :any] :any]]
                      [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:1968"}
  [list' initial fun]
  (scan-loop list' initial (list) fun))

(defn last
  "last(list: List(a)) -> Result(a, Nil)
...
(defn combinations
  ...
  {:malli/schema [:=> [:cat [:sequential :any] :int]
                      [:sequential [:sequential :any]]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:2028"}
  [items n]
  (cond
    (= n 0)
    (list (list))
...
(defn- combination-pairs-loop
  "combination_pairs_loop(items: List(a), acc: List(#(a, a))) -> List(#(a, a))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:2053"}
  [items acc]
  (if (empty? items)
    (reverse acc)
...
(defn combination-pairs
  ...
  {:malli/schema [:=> [:cat [:sequential :any]]
                      [:sequential [:tuple :any :any]]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:2049"}
  [items]
  (combination-pairs-loop items (list)))

(defn- take-firsts
  "take_firsts(rows: List(List(a)), column: List(a), remaining_rows: List(List(a))) -> #(List(a), List(List(a)))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:2112"}
  [rows column remaining-rows]
  (cond
    (empty? rows)
...
(defn- transpose-loop
  "transpose_loop(rows: List(List(a)), columns: List(List(a))) -> List(List(a))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:2096"}
  [rows columns]
  (if (empty? rows)
    (reverse columns)
...
(defn transpose
  ...
  {:malli/schema [:=> [:cat [:sequential [:sequential :any]]]
                      [:sequential [:sequential :any]]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:2092"}
  [list-of-lists]
...
(defn interleave
  ...
  {:malli/schema [:=> [:cat [:sequential [:sequential :any]]]
                      [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:2073"}
  [list']
  (-> list' transpose flatten))

(defn- shuffle-pair-unwrap-loop
  "shuffle_pair_unwrap_loop(list: List(#(Float, a)), acc: List(a)) -> List(a)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:2145"}
  [list' acc]
  (if (empty? list')
    acc
...
(defn- do-shuffle-by-pair-indexes
  "do_shuffle_by_pair_indexes(list_of_pairs: List(#(Float, a))) -> List(#(Float, a))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:2153"}
  [list-of-pairs]
  (sort list-of-pairs
...
(defn shuffle
  "shuffle(list: List(a)) -> List(a)
   ...
   ```"
  {:malli/schema [:=> [:cat [:sequential :any]] [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:2138"}
  [list']
  (-> list'
...
(defn- max-loop
  "max_loop(list: List(a), compare: fn(a, a) -> Order, max: a) -> a"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:2183"}
  [list' compare max']
  (if (empty? list')
    max'
...
(defn max'
  "max(over list: List(a), with compare: fn(a, a) -> Order) -> Result(a, Nil)
...
(defn- log-random
  "log_random() -> Float"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:2248"}
  ^double []
...
(defn- sample-loop
  "sample_loop(list: List(a), reservoir: Dict(Int, a), n: Int, w: Float) -> Dict(Int, a)"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:2225"}
  [list' reservoir n w]
...
(defn- build-reservoir-loop
  "build_reservoir_loop(list: List(a), size: Int, reservoir: Dict(Int, a)) -> #(Dict(Int, a), List(a))"
  {:gleam/src "stdlib-src/src/gleam/list.gleam:2267"}
  [list' size reservoir]
...
(defn- build-reservoir
  "build_reservoir(from list: List(a), sized n: Int) -> #(Dict(Int, a), List(a))
   ...
   the reservoir."
  {:gleam/src "stdlib-src/src/gleam/list.gleam:2260"}
  [list' n]
...
(defn sample
  "sample(from list: List(a), up_to n: Int) -> List(a)
   ...
   ```"
  {:malli/schema [:=> [:cat [:sequential :any] :int] [:sequential :any]]
   :gleam/src "stdlib-src/src/gleam/list.gleam:2207"}
  [list' n]
